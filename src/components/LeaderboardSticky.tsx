...
export default function LeaderboardSticky({ leaderboard, currentAddress }: LeaderboardStickyProps) {
    ...
    const youEntry = React.useMemo(
        () => sorted.find((e) => e.address === currentAddress) || null,
        [sorted, currentAddress]
    );

    // Entries excluding current user
    const otherEntries = React.useMemo(
        () => sorted.filter((e) => e.address !== currentAddress),
        [sorted, currentAddress]
    );
